      ...
      %      counterr=1;
            ...
            if isfield(Heatmap.(TrialNum),'OneFixationX')
             ...
             %   coordinates.(TrialNum).FixAbs=[(Heatmap.(TrialNum).OneFixationX)' (Heatmap.(TrialNum).OneFixationY)']
                ...
                ff=coordinates.(TrialNum).RelativeToCenter(1,:);
...
if isempty(fff)==0
FixationsX=fff(:,1)/pix_deg;
FixationsY=fff(:,2)/pix_deg_vert;
AllFix=[FixationsX FixationsY]
end

FixationsXinside=fff2(:,1)/pix_deg;
FixationsYinside=fff2(:,2)/pix_deg_vert;
...
fuoriarray=[(1:totaltrial)' counterOutside];
dentroarray=[(1:totaltrial)' counterInside];

trialdentro=dentroarray(dentroarray(:,2)==1);
trialfuori=fuoriarray(fuoriarray(:,2)==2);
...
% text(0,-16.5, '15^{\circ} ', 'FontSize', 20)
if isempty(fff)==0
scatter((fff(:,1)/pix_deg),fff(:,2)/pix_deg_vert, 30, [0 1 0], 'filled');
hold on
end
set (gca,'YDir','reverse')
ylabel('degrees of visual angle', 'fontsize', 28);
...
scatter(fff2(:,1)/pix_deg,fff2(:,2)/pix_deg_vert, 30, [1 0 0], 'filled');
hold on
set (gca,'YDir','reverse')
